...
write_pp_report <- function(file.in = "./modificationSpecificPeptides.txt",
    ...
    # Thang Pham
    tab.pg <- read.delim(file.in, stringsAsFactors=FALSE, fileEncoding = "UTF-8-BOM")
    ...
    # Thang Pham, 211019
    names(ev) <- str_replace(names(ev),"MS.MS..ount", "MS.MS.Count")
    ...
    cat("Done.\n")
print(colnames(tab.pg.selected))
...
hgnc <- read.delim("complete-HGNC-21Apr2016.csv",header=TRUE,sep=";",stringsAsFactors=FALSE)
...
mqver <-  extract_mq_version(paste0(TXT_DIR, "modificationSpecificPeptides.txt"))

write_normalization_table(mq.design = paste0(TXT_DIR, "experimentalDesignTemplate.txt"),
                          mq.evidence = paste0(TXT_DIR, "evidence.txt"),
                          mq.version=mqver)

write_pp_report(file.in = paste0(TXT_DIR, "modificationSpecificPeptides.txt"),
                mq.design = paste0(TXT_DIR, "experimentalDesignTemplate.txt"),
                mq.evidence = paste0(TXT_DIR, "evidence.txt"),
                kinase.file = manning.kinase.file, mapping = UniProt_data ,mq.version=mqver)
